...
export const depositPaymentService = async (
  user: IUserDocument,
  amount: number
) => {
...
export const buyPaymentService = async (
  user: IUserDocument,
  ...
  for (const product of orderedProducts) {
    await updateUserService(
      ...
      { $inc: { deposit: product.totalCost } }
    );
    await updateProductService(
      { _id: product._id },
      { $inc: { amountAvailable: -product.quantity } }
    );
  }
...
export const resetPaymentService = async (user: IUserDocument) => {
  const remainingChange = paymentChange(user.deposit || 0);
