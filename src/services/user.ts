    ...
    try {
        ...
        if (existingUser) {
            return {
                success: false,
                statusCode: 400,
                error: { path: "userName", message: `"userName":${userData.userName} already exist` }
            };
        }
    ...
    try {
        ...
        if (!user) {
            return {
                success: false,
                statusCode: 404,
                error: { path: "email or password", message: 'Incorrect username or password' }
            };
        }
...
export const updateUserWithValidationService = async (currentUsername: string, updateData: any) => {
    try {
        ...
        if (updateData.userName && updateData.userName !== currentUsername) {
            const existingUser = await getUserService({ userName: updateData.userName });
            if (existingUser) {
                return {
                    success: false,
                    statusCode: 400,
                    error: { path: "userName", message: `"userName":${updateData.userName} already exist try one else` }
                };
            }
        }

        // Update the user
        const updatedUser = await updateUserService({ userName: currentUsername }, { $set: updateData });
