...
import {
  ...
  registerUserService,
  loginUserService,
  updateUserWithValidationService,
} from '../services/user';
...
export const getUserController = async (req: Request, res: Response) => {
  ...
  return res
    .status(200)
    .send({ user: _.omit(req.user?.toObject(), 'password') });
};
...
export const updateUserController = async (req: Request, res: Response) => {
   ...
   */
  const result = await updateUserWithValidationService(
    req.user?.userName || '',
    req.body
  );
...
export const deleteUserController = async (req: Request, res: Response) => {
   ...
   * DONE: delete access token which saved in cookie
   */
  const deleted = await deleteUserService({ userName: req.user.userName });
  if (!deleted)
    return res.status(404).send({
      path: 'userName',
      message: `"userName":${req.user.userName} not found`,
    });
