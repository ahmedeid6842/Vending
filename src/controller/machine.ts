...
export const addMachineController = async (req: Request, res: Response) => {
   ...
   */
  const savedMachine = await createMachineService(req.body);
  return res
    .status(201)
    .send({ message: 'machine added succesfully', machine: savedMachine });
};
...
export const getMachineController = async (req: Request, res: Response) => {
  ...
  const machines = await getMachinesService(
    ...
    false,
    true,
    machinesPerPage * pageNumber - machinesPerPage,
    machinesPerPage
  );
...
export const getNearestMachineController = async (
  req: Request,
  res: Response
) => {
  /**
  ...
  const machines = await getMachinesService(
    { location: [req.params.longitude, req.params.latitude] },
    true,
    true,
    true,
    0,
...
export const updateMachineController = async (req: Request, res: Response) => {
  ...
  if (!updatedMachine)
    return res.status(400).send({ message: 'No machine found' });

  return res
    .status(201)
    .send({ message: 'update succesfully', updatedMachine });
};
...
export const deleteMachineController = async (req: Request, res: Response) => {
   ...
   */
  const deletedMachine = await deleteMachineService({
    _id: req.params.machineID,
  });
  if (!deletedMachine)
    return res
      .status(404)
      .send({ path: 'machine ID', message: 'no machine found' });
