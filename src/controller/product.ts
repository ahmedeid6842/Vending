...
import {
  getProductsService,
  getNearestProductService,
  addProductService,
  updateProductWithValidationService,
  deleteProductWithValidationService,
...
export const getProductController = async (req: Request, res: Response) => {
  ...
  const prodcuts = await getProductsService(
    req.query,
    true,
    true,
    productsPerPage * pageNumber - productsPerPage,
    productsPerPage
  );
...
export const getNearestProductController = async (
  req: Request,
  res: Response
) => {
  /**
  ...
  if (!products)
    return res
      .status(404)
      .send({ message: 'This product is not available in your vicinity ' });
